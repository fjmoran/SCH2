<div class="span11">
<h2>Busqueda de clientes</h2>
<h5>Realice una busqueda por el nombre o razón social del cliente</h5>
    <div class="row-fluid">
      <div class="input-append">
        <input id="busqueda" type="text">
        <button class="btn btn-primary" type="button">Buscar</button>
      </div>
    </div>


<h4>Clientes</h4>

<table class="table table-striped table-bordered table-condensed">
  <thead>
    <tr>
      <th width=10%>RUT</th>
      <th width=35%>Nombre</th>
      <th width=25%>Correo electrónico</th>
      <th width=20%>Abogado a cargo</th>
      <th width=10%>Acciones</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>60.455.345-3</td>    	
      <td>Cemento Polpaico S.A.</td>
      <td>cemento.polpaico@mail.com</td>
      <td>Nombre Apellido</td>
      <td><a href="#cli_ver"><i class="icon-eye-open" rel="tooltip" data-toggle="tooltip" title="Ver detalles" onclick="$('#cuerpo').load('pages/cli_ver.php');"></i></a>
          <a href="#cli_editar"><i class="icon-pencil" rel="tooltip" data-toggle="tooltip" title="Editar" onclick="$('#cuerpo').load('pages/cli_editar.php');"></i></a>        
          <i class="icon-remove" rel="tooltip" data-toggle="tooltip" title="Eliminar"></i>
      </td>
    </tr>
    <tr>
      <td>99.665.432-k</td>      	
      <td>Pesquera Pacific Star S.A.</td>
      <td>pesquera@mail.com</td>
      <td>Nombre Apellido</td>
      <td><a href="#cli_ver"><i class="icon-eye-open" rel="tooltip" data-toggle="tooltip" title="Ver detalles" onclick="$('#cuerpo').load('pages/cli_ver.php');"></i></a>
          <a href="#cli_editar"><i class="icon-pencil" rel="tooltip" data-toggle="tooltip" title="Editar" onclick="$('#cuerpo').load('pages/cli_editar.php');"></i></a>       
          <i class="icon-remove" rel="tooltip" data-toggle="tooltip" title="Eliminar"></i>
      </td>
    </tr>
    <tr>
      <td>99.324.554-3</td>    	
      <td>Direct TV Chile Ltda.</td>
      <td>dtv@mail.com</td>
      <td>Nombre Apellido</td>
      <td><a href="#cli_ver"><i class="icon-eye-open" rel="tooltip" data-toggle="tooltip" title="Ver detalles" onclick="$('#cuerpo').load('pages/cli_ver.php');"></i></a>
          <a href="#cli_editar"><i class="icon-pencil" rel="tooltip" data-toggle="tooltip" title="Editar" onclick="$('#cuerpo').load('pages/cli_editar.php');"></i></a>       
          <i class="icon-remove" rel="tooltip" data-toggle="tooltip" title="Eliminar"></i>
      </td>
    </tr>
    <tr>
      <td>99.434.531-1</td>     	
      <td>Telefónica Móviles Chile S.A.</td>
      <td>tefonica@mail.com</td>
      <td>Nombre Apellido</td>
      <td><a href="#cli_ver"><i class="icon-eye-open" rel="tooltip" data-toggle="tooltip" title="Ver detalles" onclick="$('#cuerpo').load('pages/cli_ver.php');"></i></a>
          <a href="#cli_editar"><i class="icon-pencil" rel="tooltip" data-toggle="tooltip" title="Editar" onclick="$('#cuerpo').load('pages/cli_editar.php');"></i></a>       
          <i class="icon-remove" rel="tooltip" data-toggle="tooltip" title="Eliminar"></i>
      </td>
    </tr>
    <tr>
      <td>99.432.455-2</td>     	
      <td>Consorcio Maderero S.A.</td>
      <td>madera@mail.com</td>
      <td>Nombre Apellido</td>
      <td><a href="#cli_ver"><i class="icon-eye-open" rel="tooltip" data-toggle="tooltip" title="Ver detalles" onclick="$('#cuerpo').load('pages/cli_ver.php');"></i></a>
          <a href="#cli_editar"><i class="icon-pencil" rel="tooltip" data-toggle="tooltip" title="Editar" onclick="$('#cuerpo').load('pages/cli_editar.php');"></i></a>      
          <i class="icon-remove" rel="tooltip" data-toggle="tooltip" title="Eliminar"></i>
      </td>
    </tr>
    <tr>
      <td>99.434.532-4</td>     	
      <td>Agrícola Los Ciruelos Ltda.</td>
      <td>agricola@mail.com</td>
      <td>Nombre Apellido</td>
      <td><a href="#cli_ver"><i class="icon-eye-open" rel="tooltip" data-toggle="tooltip" title="Ver detalles" onclick="$('#cuerpo').load('pages/cli_ver.php');"></i></a>
          <a href="#cli_editar"><i class="icon-pencil" rel="tooltip" data-toggle="tooltip" title="Editar" onclick="$('#cuerpo').load('pages/cli_editar.php');"></i></a>       
          <i class="icon-remove" rel="tooltip" data-toggle="tooltip" title="Eliminar"></i>
      </td>
    </tr>
    <tr>
      <td>12.345.322-2</td>     	
      <td>Jaime Acevedo E.</td>
      <td>jaime.acevedo@mail.com</td>
      <td>Nombre Apellido</td>
      <td><a href="#cli_ver"><i class="icon-eye-open" rel="tooltip" data-toggle="tooltip" title="Ver detalles" onclick="$('#cuerpo').load('pages/cli_ver.php');"></i></a>
          <a href="#cli_editar"><i class="icon-pencil" rel="tooltip" data-toggle="tooltip" title="Editar" onclick="$('#cuerpo').load('pages/cli_editar.php');"></i></a>      
          <i class="icon-remove" rel="tooltip" data-toggle="tooltip" title="Eliminar"></i>
      </td>
    </tr>
    <tr>
      <td>99.999.543-3</td>     	
      <td>VTR Ltda.</td>
      <td>vtr@mail.com</td>
      <td>Nombre Apellido</td>
      <td><a href="#cli_ver"><i class="icon-eye-open" rel="tooltip" data-toggle="tooltip" title="Ver detalles" onclick="$('#cuerpo').load('pages/cli_ver.php');"></i></a>
          <a href="#cli_editar"><i class="icon-pencil" rel="tooltip" data-toggle="tooltip" title="Editar" onclick="$('#cuerpo').load('pages/cli_editar.php');"></i></a>      
          <i class="icon-remove" rel="tooltip" data-toggle="tooltip" title="Eliminar"></i>
      </td>
    </tr>
    <tr>
      <td>99.678.776-3</td>     	
      <td>Fanaloza S.A.</td>
      <td>fanaloza@mail.com</td>
      <td>Nombre Apellido</td>
      <td><a href="#cli_ver"><i class="icon-eye-open" rel="tooltip" data-toggle="tooltip" title="Ver detalles" onclick="$('#cuerpo').load('pages/cli_ver.php');"></i></a>
          <a href="#cli_editar"><i class="icon-pencil" rel="tooltip" data-toggle="tooltip" title="Editar" onclick="$('#cuerpo').load('pages/cli_editar.php');"></i></a>        
          <i class="icon-remove" rel="tooltip" data-toggle="tooltip" title="Eliminar"></i>
      </td>
    </tr>
    <tr>
      <td>99.545.111-2</td>     	
      <td>Jardines Botánicos Ltda.</td>
      <td>jardin@gmail.com</td>
      <td>Nombre Apellido</td>
      <td><a href="#cli_ver"><i class="icon-eye-open" rel="tooltip" data-toggle="tooltip" title="Ver detalles" onclick="$('#cuerpo').load('pages/cli_ver.php');"></i></a>
          <a href="#cli_editar"><i class="icon-pencil" rel="tooltip" data-toggle="tooltip" title="Editar" onclick="$('#cuerpo').load('pages/cli_editar.php');"></i></a>      
          <i class="icon-remove" rel="tooltip" data-toggle="tooltip" title="Eliminar"></i>
      </td>
    </tr>    
  </tbody>
</table>

<div class="pagination pagination-centered">
  <ul>
    <li><a href="#">Anterior</a></li>
    <li class="active"><a href="#">1</a></li>
    <li><a href="#">2</a></li>
    <li><a href="#">3</a></li>
    <li><a href="#">4</a></li>
    <li><a href="#">5</a></li>
    <li><a href="#">Siguiente</a></li>
  </ul>
</div>
	
</div><!-- .span11 -->
